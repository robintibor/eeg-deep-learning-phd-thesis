...
# The Corpus was split into a training and evaluation set.
# 
...
# The inter-rater agreement on between the medical report of a certified neurologist and another annotator was 99\% for the training recordings and 100\% for the evaluation recordings.
# 
...
# 
# > Deep ConvNets already reached their best trialwise accuracies with only one minute of data used for the prediction. While the reduction of the amount of length  of the training data led to crop- and trialwise accuracy decreases on the test data, reductions in the test data did not have such an effect (see Fig. \ref{fig:reduced-time}).
...
# We show the average over all recordings for these moving averages in Fig. \ref{fig:pred-time-course}.
...
# 
# The models discovered by automated architecture optimization were markedly different from our original deep and shallow ConvNets, which were designed based on the experience in a previous study on decoding of task-related information from EEG \cite{schirrmeister_deep_2017}.
# For example, the optimized architectures used only 1.8 and 3.7 seconds of EEG data for the optimized deep and shallow ConvNet, respectively, in contrast to about 6 seconds in the original versions.
# While the improved performance of these modified architectures for the 10-fold cross-validation on the training dataset (2.1\% and 1.4\% improvement for deep and shallow ConvNets, respectively) did not generalize to the evaluation set (0.9\% and  1.5\% deterioration for deep and shallow ConvNets, respectively, see Table \ref{tab:smac-results}), the modifications to the original network architectures already provided interesting insights for further exploration:
# For example, in the case of the shallow ConvNet, the modified architecture did not use any of the original nonlinearities, but used max pooling as the only nonlinearity (see Fig. \ref{fig:smac-convnet}), a configuration we had not considered in our  manual search so far. 
...
# Power was broadly increased for the the pathological class in the low frequency bands (delta and theta range) and decreased in the beta and low gamma ranges (Fig. \ref{fig:spectral-bandpower}).
# Alpha power was decreased for the occipital electrodes and increased for more frontal electrodes.
# 
# 
# Scalp maps of the input-perturbation effects on predictions for the pathological class for the different frequency bands showed effects consistent with the power spectra in Fig. \ref{fig:spectral-bandpower}.
...
# 
# Finally, consistent with the strong usage of the delta and theta frequency range by the ConvNets as seen in the input-perturbation network-prediction correlation maps (Fig. \ref{fig:visualization-results}), `slowing` and `temporal` are the 6th and 10th most frequently occurring words in the textual reports of the pathological recordings, while never occurring in the textual reports of the normal recordings (irrespective of correct or incorrect predictions).
...
# 
# 
# Our perturbation visualizations of the ConvNets' decoding behavior showed that they used spectral power changes in the delta (0-4 Hz) and theta (4-8 Hz) frequency range, particularly from temporal EEG channels, possibly alongside other features (Fig. \ref{fig:visualization-results}). This observation is consistent both with the expectations implied by the spectral analysis of the EEG data (Fig. \ref{fig:spectral-bandpower}) and by the textual reports that frequently mentioned `temporal`  and `slowing` with respect to the pathological samples, but never in the normal ones.
